...
import { ConflictResolutionWebviewProvider } from '../provider/conflictResolutionWebviewProvider'
...
export class DiffMergeManager {
  ...
  public static async mergeSnippets(
    ...
    if (codeConflict) {
      const codeMergeResult = await this.mergeCodeContent(localSnippet.code, remoteSnippet.code, baseSnippet?.code)
      ...
      if (codeMergeResult.success) {
        ...
        if (otherConflicts.length === 0) {
          // 没有其他冲突，可以自动合并
          const mergedSnippet = this.createMergedSnippet(localSnippet, remoteSnippet, codeMergeResult.mergedCode!)
   ...
   * 检测字段级别的冲突
   */
  private static detectFieldConflicts(local: CodeSnippet, remote: CodeSnippet, base?: CodeSnippet): ConflictInfo[] {
    const conflicts: ConflictInfo[] = []

    // 检查各个字段
    const fieldsToCheck: (keyof CodeSnippet)[] = ['name', 'code', 'language', 'fileName', 'category', 'parentId']
   ...
   * 创建合并后的代码片段
   */
  private static createMergedSnippet(local: CodeSnippet, remote: CodeSnippet, mergedCode?: string): CodeSnippet {
    return {
      id: local.id,
  ...
  public static async showConflictResolutionUI(
    ...
    // 首先尝试使用新的WebView界面
    try {
      return await ConflictResolutionWebviewProvider.showConflictResolution(conflicts, localSnippet, remoteSnippet)
    } catch (error) {
    ...
    const conflictDescription = conflicts.map((c) => `• ${c.field}: ${c.description}`).join('\n')
   ...
   * 生成详细的差异报告（用于调试和用户查看）
   */
  public static generateDiffReport(localCode: string, remoteCode: string, baseCode?: string): string {
    let report = '# 代码差异报告\n\n'
