...
export class CloudOperationsManager {
  ...
  constructor(context?: vscode.ExtensionContext, storageManager?: any, gitOpsManager?: GitOperationsManager) {
  ...
  public async forcePushToCloud(
    ...
    try {
      ...
      try {
        ...
        if (errorMessage.includes('no upstream branch') || errorMessage.includes('src refspec')) {
          // 尝试设置上游分支
  ...
  public async forceImportFromGitRepo(): Promise<ForceImportResult> {
    ...
    try {
      ...
      for (const gitDirectory of gitData.directories) {
        const existingDir = currentDirectories.find((d: Directory) => d.fullPath === gitDirectory.fullPath)
      ...
      for (const gitSnippet of gitData.snippets) {
        const existingSnippet = currentSnippets.find((s: CodeSnippet) => s.fullPath === gitSnippet.fullPath)
  ...
  public async applyResolvedConflicts(): Promise<ConflictApplyResult> {
    try {
      ...
      const conflictFiles = fs
        .readdirSync(tempDir)
        .filter((file: string) => file.startsWith('conflict_') && file.endsWith('.txt'))
  ...
  private isConfigured(): boolean {
    const config = SettingsManager.getCloudSyncConfig()
    return !!(config.provider && config.repositoryUrl && (config.authenticationMethod === 'ssh' || config.token))
  }
}
