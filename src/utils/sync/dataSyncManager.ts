...
export class DataSyncManager {
  ...
  public async performSyncFlow(
    ...
    try {
      ...
      if (isEmptyLocalData && hasRemoteData) {
        ...
        try {
          ...
          const needsWrite = this.checkIfVSCodeDataDiffersFromGit(
            localSnippets,
            localDirectories,
            currentGitData.snippets,
            currentGitData.directories
          )
      ...
      if (
        ...
        hasChangesToCommit =
          statusAfterWrite.staged.length > 0 ||
      ...
      if (needsMerge) {
        ...
        try {
          ...
          if (this.storageManager) {
            ...
            const updateResult = await this.performSafeStorageUpdate(mergedData.snippets, mergedData.directories)
          ...
          // 检查是否是未完成的合并错误
          if (
            errorMessage.includes('unfinished merge') ||
            errorMessage.includes('Exiting because of unfinished merge')
          ) {
            return {
              success: false,
        ...
        if (isWorkingDirClean && !needsMerge) {
          ...
          try {
            ...
            const directoriesMatch =
              JSON.stringify(vscodeNormalizedDirectories) === JSON.stringify(gitNormalizedDirectories)
            ...
            console.log(
              `   Git仓库: ${gitRepoData.snippets.length} 个代码片段, ${gitRepoData.directories.length} 个目录`
            )
            ...
            if (isDataConsistent) {
                ...
                for (let i = 0; i < Math.max(vscodeNormalizedSnippets.length, gitNormalizedSnippets.length); i++) {
                  ...
                  if (!vscodeSnippet) {
                    ...
                    if (vscodeSnippet.code !== gitSnippet.code) {
                      console.log(
                        `     内容长度: ${vscodeSnippet.code?.length || 0} vs ${gitSnippet.code?.length || 0}`
                      )
                    }
                  }
                }
              ...
              const hasChangesToCommit =
                statusAfterStaging.staged.length > 0 ||
            ...
            const hasSafeChangesToCommit =
              statusAfterSafeStaging.staged.length > 0 ||
  ...
  private async performIntelligentMerge(
    ...
    try {
      ...
      console.log(
        `代码片段合并结果: ${snippetMergeResult.merged.length} 个, ${snippetMergeResult.conflicts.length} 个冲突`
      )

      // 合并目录
      const directoryMergeResult = this.mergeDirectories(localDirectories, remoteDirectories)
      console.log(
        `目录合并结果: ${directoryMergeResult.merged.length} 个, ${directoryMergeResult.conflicts.length} 个冲突`
      )
      ...
      if (manualSnippetConflicts.length > 0 || manualDirectoryConflicts.length > 0) {
        console.log(
          `检测到需要手动解决的冲突: ${manualSnippetConflicts.length} 个代码片段冲突, ${manualDirectoryConflicts.length} 个目录冲突`
        )
        ...
        try {
          ...
          if (manualHandleResult.resolvedSnippets) {
            ...
            snippetMergeResult.conflicts = snippetMergeResult.conflicts.filter(
              (c) => !manualHandleResult.resolvedSnippets!.some((resolved) => resolved.fullPath === c.fullPath)
            )

            console.log(
              `冲突解决后，剩余 ${snippetMergeResult.conflicts.length} 个代码片段冲突（原有 ${originalConflictCount} 个）`
            )
          } else {
      ...
      const localOnlySnippets = localSnippets.filter(
        (local) => !remoteSnippets.find((remote) => remote.fullPath === local.fullPath)
      )
      const remoteOnlySnippets = remoteSnippets.filter(
        (remote) => !localSnippets.find((local) => local.fullPath === remote.fullPath)
      )
      const localOnlyDirectories = localDirectories.filter(
        (local) => !remoteDirectories.find((remote) => remote.fullPath === local.fullPath)
      )
      const remoteOnlyDirectories = remoteDirectories.filter(
        (remote) => !localDirectories.find((local) => local.fullPath === remote.fullPath)
      )
          ...
          if (!isIdentical) {
            ...
            if (vscodeSnippet.code !== gitSnippet.code) {
              ...
              if (vscodeSnippet.code?.charAt(0) !== gitSnippet.code?.charAt(0)) {
                console.log(
                  `       ⚠️ 开头字符不同: VSCode="${vscodeSnippet.code?.charAt(0)}" vs Git="${gitSnippet.code?.charAt(
                    0
                  )}"`
                )
              }
   ...
   */
  private async performSafeStorageUpdate(
    ...
    try {
      ...
      // 【步骤2】分析需要的变更操作
      console.log(`🔍 分析存储变更...`)
      const changeSet = await this.analyzeStorageChanges(
        backupData.snippets,
        backupData.directories,
        snippets,
        directories
      )
      ...
      console.log(`📊 变更分析结果:`)
      console.log(
        `   代码片段: 新增${changeSet.snippetsToAdd.length}, 更新${changeSet.snippetsToUpdate.length}, 删除${changeSet.snippetsToDelete.length}`
      )
      console.log(
        `   目录: 新增${changeSet.directoriesToAdd.length}, 更新${changeSet.directoriesToUpdate.length}, 删除${changeSet.directoriesToDelete.length}`
      )
      ...
      if (backupData) {
        ...
        try {
          ...
          return {
            success: false,
            error: `更新失败且回滚失败: 原始错误=${error instanceof Error ? error.message : '未知错误'}, 回滚错误=${
  ...
  private async analyzeStorageChanges(
    ...
    for (const targetSnippet of targetSnippets) {
      ...
      if (!currentSnippet) {
        currentSnippet = currentSnippets.find(
          (s) => s.name === targetSnippet.name && s.filePath === targetSnippet.filePath
        )
      ...
      if (!currentSnippet) {
        ...
        const deleteIndex = snippetsToDelete.findIndex((s: CodeSnippet) => s.fullPath === currentSnippet!.fullPath)
        if (deleteIndex >= 0) {
    ...
    console.log('📊 变更分析完成:')
    console.log(
      `   代码片段: 新增${snippetsToAdd.length}, 更新${snippetsToUpdate.length}, 删除${snippetsToDelete.length}`
    )
    console.log(
      `   目录: 新增${directoriesToAdd.length}, 更新${directoriesToUpdate.length}, 删除${directoriesToDelete.length}`
    )
  ...
  private async applySnippetChanges(changeSet: {
    ...
    for (const snippet of changeSet.snippetsToUpdate) {
      ...
      if (existingSnippet) {
        console.log(
          `   找到现有片段: 创建时间=${existingSnippet.createTime}, 内容长度=${(existingSnippet.code || '').length}`
        )
  ...
  private async validateStorageUpdate(
    ...
    try {
      ...
      if (actualSnippets.length !== expectedSnippets.length) {
        ...
        return {
          isValid: false,
          reason: `代码片段数量不匹配: 期望${expectedSnippets.length}, 实际${actualSnippets.length}${
            duplicates.length > 0 ? `, 发现${duplicates.length}个重复片段` : ''
          }`,
        }
      }
   ...
   * 【新增】回滚VSCode存储更改
   */
  private async rollbackStorageChanges(backupData: {
    snippets: CodeSnippet[]
    directories: Directory[]
  }): Promise<void> {
    if (!this.storageManager) {
    ...
    try {
      ...
      console.log(
        `✅ 回滚完成: 恢复了 ${backupData.snippets.length} 个代码片段和 ${backupData.directories.length} 个目录`
      )
    } catch (error) {
  ...
  private async performReinitializedSync(
    ...
    try {
      ...
      if (
        localDataCount > 0 &&
        remoteCheckResult.remoteHasData &&
        !options?.forceSmartMerge &&
        !options?.forceUseLocal &&
        !options?.forceUseRemote
      ) {
      ...
      // 【重新初始化策略】步骤1: 确保本地 VSCode 拥有最新的远程数据
      if (remoteCheckResult.remoteHasData && !remoteCheckResult.isRemoteEmpty) {
        console.log('📥 步骤1: 确保本地 VSCode 拥有最新的远程数据...')
        ...
        try {
          ...
          if (remoteUpdates.hasUpdates) {
            ...
            console.log('✅ 远程无新更新，本地 Git 仓库已是最新')
          }

          // 1.3 【核心修复】检查本地 VSCode 是否为空，如果是，则必须从 Git 导入数据
          // 即使没有远程更新（比如刚克隆完），如果 VSCode 是空的，也需要导入
          const isEmptyVSCode = localSnippets.length === 0 && localDirectories.length === 0
          if (isEmptyVSCode) {
            console.log('📥 检测到本地 VSCode 数据为空，正在从 Git 仓库导入初始数据...')
            const gitData = await fileSystemManager.readFromGit()
            
            if (gitData.snippets.length > 0 || gitData.directories.length > 0) {
              if (this.storageManager) {
                const importResult = await this.performSafeStorageUpdate(gitData.snippets, gitData.directories)
                if (importResult.success) {
                  console.log(`✅ 成功导入 ${gitData.snippets.length} 个片段和 ${gitData.directories.length} 个目录`)
                  // 更新本地变量，防止后续步骤 2 写入空数据
                  localSnippets = gitData.snippets
                  localDirectories = gitData.directories
                } else {
                  throw new Error(`从 Git 导入数据到 VSCode 失败: ${importResult.error}`)
                }
              }
            } else {
              console.log('📝 Git 仓库中也没有有效数据，跳过导入')
            }
          } else if (remoteUpdates.hasUpdates) {
            // 如果 VSCode 不为空且有更新，执行智能合并
            const remoteData = await fileSystemManager.readFromGit()
            ...
            if (mergeResult.needsVSCodeUpdate && this.storageManager) {
              ...
              // 更新本地变量
              localSnippets = mergeResult.mergedSnippets
              localDirectories = mergeResult.mergedDirectories
   ...
   */
  private async detectRepositoryReinitialization(
    gitOpsManager: any,
    remoteCheckResult: RemoteCheckResult
  ): Promise<boolean> {
    try {
      ...
      if (this.storageManager) {
        try {
          ...
          console.log(
            `   VSCode本地数据量: ${localSnippets.length} 个代码片段 + ${localDirectories.length} 个目录 = ${localVSCodeDataCount} 项`
          )
        } catch (error) {
      ...
      if (remoteCheckResult.remoteHasData) {
        ...
        // 【优化】新设备识别逻辑
        // 只要本地VSCode数据为空，且远程有数据，就应该视为重新初始化（需要拉取）
        // 即使本地有一些Git历史（可能是之前的残留），只要VSCode数据为空，就应该优先拉取远程
        if (localVSCodeDataCount === 0 && remoteCheckResult.remoteHasData) {
          console.log('🔄 检测结果: 确认为新设备或数据重置场景')
          console.log(`   理由: VSCode无数据(${localVSCodeDataCount}项)，远程有数据，需要拉取`)
          return true
        }
        ...
        console.log(
          `   数据状况: 本地提交(${localCommitCount})，工作区文件(${gitStatus.files.length})，VSCode数据(${localVSCodeDataCount}项)`
        )
        return false
